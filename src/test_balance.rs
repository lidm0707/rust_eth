...
async fn main() -> Result<(), Box<dyn Error>> {
    ...
    let provider: Arc<Provider<Http>> = Arc::new(Provider::<Http>::try_from("http://127.0.0.1:8545")?);
    ...
    let mut file = File::open("./src/build/src_contract_ERC20_sol_MooCoin.abi").expect("Unable to open file");
    let mut content = String::new();
    file.read_to_string(&mut content).expect("Unable to read file");
    
    let erc20_abi: Abi = serde_json::from_str(&content)?;
    // Initialize the contract
    let contract: ContractInstance<Arc<Provider<Http>>, _> = Contract::new(token_contract_address, erc20_abi, Arc::clone(&provider));
